...
final class FrameRenderTests: XCTestCase {
    ...
    func testRendersEveryStateOfTheRedesignSurfaces() throws {
        ...
        let rowSize = NSSize(width: DashboardSurface.size(for: .classic).width - 20, height: 120)
        let steps: [(String, MakeActiveRow, String)] = [
            ("offer", MakeActiveRow(name: "dJormun", provider: .claude, loginDead: false, isPending: false, note: nil,
                                    onBegin: {}, onConfirm: {}, onCancel: {}), "the offer on a viewed non-owner"),
            ("confirm", MakeActiveRow(name: "dJormun", provider: .claude, loginDead: false, isPending: true, note: nil,
                                      onBegin: {}, onConfirm: {}, onCancel: {}), "confirmation with the switch cost"),
            ("confirm-dead", MakeActiveRow(name: "Ai", provider: .claude, loginDead: true, isPending: true, note: nil,
                                           onBegin: {}, onConfirm: {}, onCancel: {}), "confirmation on a dead login"),
            ("outcome", MakeActiveRow(name: "dJormun", provider: .codex, loginDead: false, isPending: false,
                                      note: DashboardFormatting.outcome(.activated, name: "dJormun"),
                                      onBegin: {}, onConfirm: {}, onCancel: {}), "outcome note after a switch"),
        ]
