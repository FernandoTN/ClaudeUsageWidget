...
| 24 | `adoptSystemLoginByIdentity` / `adoptCodexLoginByAccountId` | pointer moves, Viewing untouched, no user-visible signal | (fine) + notify | post one `.providerOwnerChangedExternally` notification per episode (object: provider + new owner id) so the UI can show "Active for Claude changed outside the app: now Lark" |
