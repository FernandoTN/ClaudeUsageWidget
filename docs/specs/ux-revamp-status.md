...
| 3d-delete: the eight legacy pages, `ProfileSelectionRow`, `SettingsButton`, the credential mini-cards, the eight `SettingsSection` cases and the two classification flags removed; `SettingsRoute.legacyAliases` keeps every old raw value decoding; ⇄ menu routes typed (repair lands on the Login tab) | `feat/ux-revamp-3d-delete` | #104 | **owner approved 2026-09-04** — rebased on main, Release + full suite, then merged (sha in the next docs commit) |
