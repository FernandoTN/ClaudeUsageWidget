...
| 3a Settings › Active & Auto-switch (`SettingsSection.activeAccounts`, `ActiveSwitchView`: three provider cards mirroring the ⇄ menu, enable + thresholds, hand-off queue with provider filter, eligibility list as the one location), R2-2 amber `▲ W` marks, R2-4 fixture emails, roster header in WORDS (owner ruling), neutral queued pill (redesign #95 made `.action` link blue) | `feat/ux-revamp-3a-active` | draft | design pass §12.3 |
| 3b Alerts page + `fleetAlertDefaults_v1`; 3c Display + Advanced + `SettingsKeyRegistry` + route aliases; 3d remove legacy pages | — | — | after 3a |
