...
| 2026-09-04 15:10 | V1/V2 merged (`9040fbc`). Redesign's six-state taxonomy (#138) mapped my consumers mechanically; two literal leftovers fixed here: maxed marks in the row's role colour, census merged by hue |
| 2026-09-04 14:00 | Owner findings on the live app at 19 profiles (relayed by the orchestrator): V1 reset-strip labels overlapping and spilling into the next section; V2 the roster census clipped and "1 account" misleading. Fixed with a collision-aware strip that reserves its height and a one-line census with "N identified"; verified with a 19-profile fixture (spec §12.10) |
| 2026-09-04 13:20 | Post-plan: the ⇄ item is pinned by a stable autosave name (`cuw.selector`, preferred position 50 seeded only when absent) alongside the redesign's per-role group names — the deployed host ignores creation order; §2.1's "no autosaveName" ruling superseded (rotating per-tile names were the July failure, not naming) |
| 2026-09-04 12:40 | The owner approved deleting the legacy Settings pages; #104 rebased twice (status doc, then the redesign's default-layout commit), merged as `a6a0c2b`. S2 (#121) and F1 (#126) landed earlier. **The UX revamp's staged plan is complete**: every stage from §8 is on main and deployed or queued for the next restart; nothing open |
...
| 3d-delete: the eight legacy pages, `ProfileSelectionRow`, `SettingsButton`, the credential mini-cards, the eight `SettingsSection` cases and the two classification flags removed; `SettingsRoute.legacyAliases` keeps every old raw value decoding; ⇄ menu routes typed (repair lands on the Login tab) | `feat/ux-revamp-3d-delete` | #104 | **merged** `a6a0c2b` (owner approved 2026-09-04 in the sibling session; 547/0 on the exact merged tree) |
...
| Fix (2026-09-04 16:38 incident): a manual Codex switch onto the focused profile left the fleet tile on the old owner for ~30 s and could be silently reverted. `FleetRepaintScheduler` repaints the fleet item on every `.providerOwnerClaimed` (coalesced per turn, held while `isSwitchingProfile`, one paint on completion — the sweep-end paint no longer lands mid-switch); `SummaryRenderKey.activeId`; the apply FAILS CLOSED (`ActivationOutcome.credentialWriteFailed`: no pointer claim on a thrown write, Codex reads `auth.json` back and requires the target `account_id`; walk → retry next sweep; ⇄/dashboard show the outcome; automatic path notifies); `ProfileManager.externalRepairGrace` = 90 s after a user-initiated claim during which `adoptCodexLoginByAccountId` / `adoptSystemLoginByIdentity` hold the pointer (one log line per claim); an adopted external rewrite now posts a user notification ("<Provider> login changed outside the app: now <profile>"). The ⇄ selector already applied immediately (`activateProfileDetailed(userInitiated: true)` — the 6.6 s in the log was the modal confirmation). | `fix/codex-switch-repaint` | #153 | **merged** `9bed05e`, deployed 2026-09-04 17:50 (pid 11565), suites 576 / 0 before and after the merge |
| Perf + audit (2026-09-05 idle-CPU diagnosis, parts B + D): `ProfileStore.loadProfiles()` decoded the 38 KB `profiles_v3` JSON 44 times a minute (~22 calls per sweep) — the decoded roster is now kept beside the bytes it came from and byte-identical reads are served without a decode (`decodeStoredProfiles`; every writer of the key misses it by construction; cfprefsd fallbacks untouched). `NotificationManager.deliver` logs `Notification: <category> id=<dedupe key> title=… profile=…` per send and keeps the last 64 in `recentDeliveries`; the ledger is isolated under XCTest; inferred-throttle and projected-exhaustion notices post once per (profile, session window). The PR body carries the sender/dedupe audit, the attribution of the night's 48 posts (35 = `sendProfileAlert` bands + reset notices, 8 owners × 4), the fixture-run counts (`NotificationAuditTests`) and a PROPOSAL for fleet defaults under auto-switch (owner's call, not implemented). | `fix/idle-cpu-audit-b` | #161 | **merged** `68933b8` (squash), deployed 2026-09-05 10:38:24 as pid 61652; suites 619 / 0 before and after the merge; the deploy probe window showed 1 `Loaded 23 profiles` decode (was ~100) |
| Fix (2026-09-05, part C): `StormWatchdog` fired 270 strikes overnight at a 1.5 % threshold set above a 0.0 % idle that predates the sweep being visible; the app's own steady state measured 1.5 % of a core after #160 (0.9 s CPU / 60 s, pid 91164). `StormWatchdogPolicy` (pure, tested) with the threshold at 4.5 % = 3× that baseline; the alarm names the measured mean over the hot run and prescribes quitting ONLY behind the storm signature (windows or tracking areas grew during the episode); routed through `NotificationManager.deliver`. Gates unchanged (45-min floor, 20-min bank, 6 h re-post, pause override, one remediation per episode). | `fix/idle-cpu-watchdog` (rebased onto `68933b8` after #161 merged; #162 closed itself with its deleted base branch) | #163 | **merged** `fd626cf` (squash), deployed 2026-09-05 10:47:52 as pid 39798; suites 623 / 0 before and after the merge; baseline re-measured on pid 61652 (main `68933b8`) at 2.5 min uptime: 0.9 s CPU per 60 s = 1.5 %, same as after #160. Measured effect of the four parts: 10.4 % → 1.5 % of a core, 44 → ~2 decodes/min, every notification attributable. The notification-default proposal (#161 body §7) stays with the owner. |
| Fix (2026-09-05 15:47 incident): four transcripts said "You've reached your Fable limit" (no reset in the text) while the owner read session 89 % / Fable 100 %; the tripwire corroborated against the SESSION window ("live read says 89% — ignoring"), then stamped the event as a session hit with a 30-minute placeholder reset. `LimitWindow` (session / weekly / fableWeekly / unknown) is classified from the message text (`classifyWindow`; the dated "resets Aug 22 at 3pm (Zone)" form now parses); attribution matches the NAMED window's stamp and, with no reset in the text, the window's STATE (owner whose matching window is at the limit, threshold capped at 95 %); corroboration reads the named window (a header rescue is `unavailable` for Fable — no per-model windows); the stamp lands on the named window (`ClaudeUsage.stampLimitHit`: Fable 100 → `.weeklyHit`, capacity returns at the Fable reset); the 30-minute placeholder is removed; `Incident.window` + Insights wording. Spec: `docs/specs/transcript-limit-attribution.md` ("The window"). | `fix/transcript-limit-class` | #165 | **merged** `c22d614` (squash) 2026-09-05; 9 new tests, suites 632 / 0 before and after the merge, Release green; deploy staged by the orchestrating session behind the next Claude switch (the owner sat at 90 % session when the gate closed) |
| Fix (2026-09-04 22:53 incident): a transcript rate-limit event from the PREVIOUS Claude owner's still-running sessions ("resets 3:40am" = dFr's 03:39:59 window) was attributed to the new owner by the wall clock, dJormun was stamped exhausted while its own endpoint read 22 % in the same second, and the fleet hopped again to Google one minute after the switch. `TranscriptLimitAttribution` attributes by the reset time the event names against the owners' cached windows (previous owners from the switch history within 15 min, ±3 min tolerance; a previous-owner match records the hit against that profile and never switches), the blamed current owner is re-measured live before any switch (headroom → `contradicted`, ignored), and a 120 s post-switch grace refuses unmatched events; `FleetInsights.Incident.tripwire` carries the disposition and the Insights row says "429 from the previous owner — ignored". `SwitchEvent.from` now records the outgoing OWNER, not the focused profile (every automatic row of the day read 'xLucifer(dev)'). Spec `docs/specs/transcript-limit-attribution.md`. | `fix/transcript-limit-attribution` | #158 | **merged** `4352b40` (squash), deployed 2026-09-04 23:25:01 (pid 73406), suites 612 / 0 before and after the merge |
| Weekly window priming, Codex only (owner ask 2026-09-09; scope narrowed to Codex by owner decision the same day): a Codex weekly window opens on the first real request after the previous one ended, so an idle account has no window and its next reset lands 7 days after whoever uses it next (live 2026-09-09: xLucifer (dev)'s stored reset drifted with every fetch — the parser invented `now + 7 d` whenever `wham/usage` carried no window). `ClaudeUsage.weeklyWindowOpen` now says so (parser sentinel + healer projection, marked `~`), `WeeklyWindowPrimer` ticks at sweep end — one non-owner Codex profile per sweep, 2–10 min deterministic jitter after the window is seen closed, once per window, one retry after 30 min, never mid-switch — and runs `CODEX_HOME=<profile's isolated home> codex exec … "Reply with exactly OK"` off the main actor (stored login written to the isolated home first, 90 s timeout, stdin closed, rotation adopted back, never the shared daemon or the default home), then verifies through a forced fetch: `Prime: codex '<name>' — window reset moved none → <date>, used N%`. Keys `weeklyPrimePolicy_v1` (toggle "Prime Codex weekly windows after reset", default ON; "Never prime" list) and `weeklyPrimeLedger_v1`; dashboard roster row + inspector Overview print `primed HH:MM · resets <date>` / `prime pending HH:MM`, Overview gains **Prime now**, Insights gains a `primed` incident, one INFO notice per spent episode. Spec `docs/specs/weekly-window-priming.md`. | `feat/weekly-window-priming` | — | draft; 15 new tests |
