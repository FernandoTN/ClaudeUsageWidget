...
| **`weeklyPrimePolicy_v1`** | SharedDataStore | **new** (2026-09-09), journaled + shadowed | `WeeklyPrimePolicy` JSON `{ codexEnabled: Bool, neverPrime: [UUID] }`; **an absent key reads as Codex priming ON with nobody excluded**; each field decodes with its default (`docs/specs/weekly-window-priming.md`) | Active & Auto-switch › Weekly window priming |
| **`weeklyPrimeLedger_v1`** | SharedDataStore | **new** (2026-09-09), journaled + shadowed | `[profile UUID: WeeklyPrimeRecord]` JSON — per profile the closed-window episode (`episodeObservedAt`, `attempts`, `lastAttemptAt`, `lastOutcome`, `lastDetail`) and the last prime that opened a window (`lastPrimedAt`, `primedForWindowEndingAt`, `lastVerifiedUsedPercent`); rows of deleted profiles are pruned on the next tick; never a credential | Dashboard roster row; Accounts › Overview ("primed HH:MM · resets …", "prime pending") |
| **`fleetCapacitySeries_v1`** | SharedDataStore | **new** (2026-09-22), journaled + shadowed | `[[epochSeconds, pool, accounts, scheduleKey]]` JSON — the Claude weekly pool sampled at most every 5 min at sweep end, 24 h kept (≤ 300 rows, ~12 KB); numbers only, no names, never a credential (`docs/specs/fleet-capacity-forecast.md`) | Menu bar (Claude `pool·runway`); Dashboard › Weekly capacity |
