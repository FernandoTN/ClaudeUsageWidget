...
| 2026-09-16 → | **Per-account "Show in the menu bar"** (owner ask, sibling session, `feat/menubar-account-visibility`): seven held dev accounts should leave the bar now and come back with a click later. `isSelectedForDisplay` was the wrong lever — the fleet layouts ignore it and it gates the fetch sweep, so a deselected account goes stale and becomes a blind auto-switch pick. New `Profile.hideFromMenuBar` (`Bool?`, nil = shown, no migration): `multiProfileCreationOrder` drops hidden accounts unless `includeHidden` (dashboard, next-account hotkey) or the id is in `alwaysShown` (the provider-active accounts); `fleetPaintOrder` feeds the fleet block, so dots, counts, `+N`, width and the mark number (`ProviderSummary.markCount`: 22 with 7 hidden → `Cl 15`) follow; `barProviders` = selected AND shown decides which group items exist. Tooltip ends `· 7 hidden`. Hidden accounts keep their readiness (the candidate row can still name one) and stay in the sweep and the walk (`sweepPopulation`, `candidateRejection`, regression-tested). UI: Accounts › Monitoring toggle (the old one is now "Monitor this account"), Display › Menu bar accounts bulk list with "Show all" per provider, dimmed roster rows. Frames: `fleet-dots-21` (before) vs `fleet-dots-22-hidden-7` / `fleet-counts-22-hidden-7`. |
| 2026-09-15 21:13 → | **Fleet-dot cap 20 → 24** (owner report, sibling session, `fix/fleet-dot-cap-26`): the Claude fleet reached 22 accounts and the bar folded its 21 others into 18 dots + `+3` — "make it so that the max is now 24 instead of 20". `FleetBlockGeometry.maxDotColumns = 12` replaces the dead `dotsPerRow` (a constant `dotGrid` never applied) and `ProviderSummary.maxDotMembers = 2 × maxDotColumns`; `dotMembers()` now shares `shownDotCount` instead of its own `− 2`. Widths from `dotMatrixWidth`: 21 others → 75 pt (was 77 with the `+3`), 24 → 82 pt, 25 or more → 91 pt (22 dots + `+N`; was 77). Dot pitch, diameter and row pitch untouched — shared with the Codex / Grok blocks, and two rows still fill the 22 pt bar. Fixtures `fleet-dots-21` (the live fleet, no `+N`) and `fleet-dots-overflow` (30 accounts). |
| 2026-09-04 21:25 → | **Queue resets** (owner ask, sibling session): the three collapsed roster rows were the bar's LEFTMOST dots (farthest resets, unmeasured first) while the header promised "soonest weekly reset first" — the owner read them as the queue. The roster is now three bands (`RosterGroup`): next up in the walk's own order (predicted target, queue, soonest weekly reset), capacity returns (exhausted accounts by `capacityReturnsAt`, the LATEST reset among hit windows — every window must have headroom), not switchable (bar order). Every row prints its weekly countdown on a third line (`ResetCountdown`: W, or F when Fable alone is exhausted; "~" on a projected boundary via the new `ClaudeUsage.weeklyResetProjected` / `fableWeeklyResetProjected` stamps healed in, cleared by a real stamp; "reset unknown" on the sentinel), absolute time on hover; the active card's gauges wear the "~" too. Frames `dashboard-queue-resets[-expanded]-*`. First render wrapped the gauges (line 2's 172 pt cannot hold three gauges and a clock) → third line. **#156 merged `5ea0099`, deployed 22:00:37 (pid 76880), design pass accepted.** Follow-up B3.1: a session-exhausted row now leads its third line with the binding reset ("S resets in 2h 10m · W in 2 days") so the sort key and the printed countdown agree. |
| 2026-09-04 14:48 | **Placement settled, field-confirmed.** #149 deployed as pid 50250; every probe identical: `fleet=exposed [x=1034 w=215 h=33 len=212 … img=1/2→1/5 hosts=codex,grok,claude+⇄]` — one item, no order field, the ⇄ no longer an auxiliary entry, repaints in place (one assignment, four repaints). The bar reads Codex · Grok · Claude · ⇄ by construction. Open: nothing, unless an overflow appears (`fleet=hidden`) — that sample starts Stage C's ladder. |
| 2026-09-04 16:38 → fix | **Fleet item did not follow a manual Codex switch** (owner report; log at ms resolution): the item paints from the live pointers but only `$activeProfile` (deduped on id), the sweep's final paint, the 30 s timer and `.credentialsChanged` triggered a paint — a switch onto the already-focused profile moved no focus, and the aborted sweep's paint read the pointer 1 ms before the claim (`.984` vs `.986`), so the tile kept the old owner until `16:39:10`. Fix (`fix/codex-switch-repaint`, sibling session): `FleetRepaintScheduler` — one coalesced paint per turn on every `.providerOwnerClaimed`, held while a switch is in flight, one paint when it completes; the sweep-end paint defers to it while switching; `SummaryRenderKey` gains `activeId` (two never-painted owners had an equal key). Log line to look for: `MenuBarManager: fleet repaint (ownerChanged|switchCompleted|sweepEnd)`. |
| 2026-09-04 14:30–15:30 | Placement, decisive negative sample #2 (fixes session, 14:19:35 restart): the per-role pins held on four launches and not on the fifth (`codex < claude < grok`, saved positions intact, no repair fired). Last rung of the ladder built: ONE status item hosts the three provider strips (Codex, Grok, Claude, 6 pt gaps) plus the ⇄ as its rightmost segment; per-segment clicks (`hostedHit`), per-segment tooltips, one exposure entry (`fleet=… hosts=…`); `CUW_SEPARATE_GROUPS=1` restores separate items. Spec §2.7. Overflow now hides the whole fleet at once — Stage C's ladder has to shrink the image. |
...
| B3 queue resets | `feat/dashboard-queue-resets` | `RosterGroup` bands and `DashboardSnapshot.orderedRoster` / `rosterGroup` / `weeklyReset(for:)` / `capacityReturnsAt`; `RosterRow.group / weeklyReset / capacityReturnsAt`; `ResetCountdown`; `WindowGauge.projected`; `ClaudeUsage.weeklyResetProjected` + `fableWeeklyResetProjected` (stamped by `healMissingResetStamps`, cleared by `mergingHeaderMeasurement` and any fresh parse); `MenuBar/DashboardRosterBands.swift` (`DashboardFormatting.resetCountdown` / `resetHelp` / `bandLabel`, `RosterBandLabel`, `RosterResetText`); the roster header reads "next up, then soonest reset"; `DashboardView.rostersExpanded` harness seed; strings `dashboard.reset_*` / `dashboard.band.*`; legend + README updated. `DashboardQueueResetTests` (14). | #156 merged `5ea0099`, deployed 2026-09-04 22:00 (pid 76880); design pass on the light / dark / expanded frames accepted as shipped |
| B3.1 binding reset | `fix/dashboard-binding-reset` | Follow-up to the orchestrator's design nit: a session-exhausted row under "capacity returns" sorted by its session reset but printed the weekly one. `RosterRow.sessionReturnsAt` (the band's key when the session window or an affirmed stamp binds) and `DashboardFormatting.resetLine` — "S resets in 2h 10m · W in 2 days" (`resetCountdown(short:)` drops the verb on the second half); `resetHelp(row)` names both boundaries. `DashboardQueueResetTests` (15). | PR |
