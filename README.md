...
**Weekly capacity (Claude).** Right of the next-switch hint, the Claude block shows the fleet's weekly **pool** and its **runway**: `609·31h` means 609 points left across the accounts the auto-switch can use (one point is one percent of one account's weekly window, so 24 accounts are worth 2 400), gone in 31 hours at the measured burn. The runway appears only when the burn is measured (at least six samples over an hour) and above the sustainable rate. That rate is accounts × 100 / 168 points per hour, since every account renews once a week. Otherwise the pool shows alone. The text only uses space the hint leaves free, so the block never gets wider. When space is short it drops to the pool, and in the smallest blocks it is omitted. The block's tooltip spells out the pool, the burn against the sustainable rate, and the runway. Details: `docs/specs/fleet-capacity-forecast.md`.
...
**Ignoring the Fable weekly window.** A fleet deliberately running on another model can spend its Fable week long before the rest of its quota, and the switch then treats an account with real capacity as exhausted — and refuses it as a target. **Settings › Active & Auto-switch › “Ignore the Fable weekly limit when switching accounts”** takes that window out of the decision on both sides at once: such an account keeps serving, and can still be switched to, while its session and all-models weekly windows have headroom. Off by default, and the session and all-models weekly thresholds still apply.

The display follows the same rule, so the bar never paints as limit-hit an account the switch is about to take: while the setting is on, a spent Fable window alone leaves the dot green (the *light* shade — the spent window still shows in how much capacity is left), keeps the tile label out of red, files the row under **next up** with its all-models weekly countdown, and makes the account clickable in the ⇄ **Switch to ▸** submenu. Genuine exhaustion — the all-models weekly window, or the 5-hour session — reads exactly as it always did. Each account's Fable bar is drawn as measured throughout, and the ⇄ menu's policy row says `Fable ignored` while the setting is on.
...
**Weekly window priming (Codex).** A Codex weekly window is rolling: it opens on the first real request after the previous one ended. Until then the usage endpoint reports a placeholder — 0 % used, a countdown equal to the full window length, a reset stamp that follows the clock — so an idle account's next reset lands 7 days after whenever the rotation happens to reach it; the roster row says `W no window (idle)`. When a non-active Codex account's window is seen closed, the app sends one tiny request through the CLI 2–10 minutes later (`codex exec` in the account's own isolated `CODEX_HOME`, never the shared daemon, never the active account) so the 7-day clock runs while the account waits; the next fetches verify that the countdown actually started, and the roster row and the account's Overview say `primed 08:12 · resets Sep 16` (measured, never synthetic), `prime sent · verifying` or `prime pending`. Once per window, one retry after 30 minutes, one notice if both fail. On by default in **Settings › Active & Auto-switch**, with a per-account "Never prime" list and a **Prime now** action in the Overview. Claude accounts are not primed. Spec: `docs/specs/weekly-window-priming.md`.
